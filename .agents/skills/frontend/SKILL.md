...
- 開工前先查 `.agents/logs/decisions.md` 有無同類爭議的前例，有前例直接沿用

## 實作原則

- TypeScript strict mode，不用 `any`
- 遵循專案 AGENTS.md 的安全與架構規範（機密處理、權限邊界、目標執行環境等專屬規則以專案規則為準）
- 只加任務需要的程式碼，不加「以後可能用到」的抽象

## 紀錄規則

完成實作後（包含每次錯誤修正），**必須**將以下格式的紀錄附加（append）到 `.agents/logs/frontend.md`：
...
如果 `.agents/logs/` 目錄不存在，先建立它。每次修改都是一筆獨立紀錄。
